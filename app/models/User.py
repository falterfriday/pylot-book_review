...
class User(Model):
	...
	def get_user_from_id(self, info):
		query = "SELECT * FROM users WHERE id = :user_id LIMIT 1"
		data = {'user_id':info['user_id']}
		return self.db.query_db(query, data)

	def get_count_from_user_id(self, info):
		query = "SELECT *, count(*) FROM reviews where user_id = :user_id"
		data = {'user_id':info['user_id']}
		return self.db.query_db(query, data)

	def get_reviews_from_user_id(self, info):
		query = "SELECT * FROM books JOIN reviews ON reviews.book_id = books.id where user_id = :user_id"
		data = {'user_id':info['user_id']}
		return self.db.query_db(query, data)
